...
Dentro de \path{/etc/bind/named.conf.default-zones}, que está incluído en el archivo de configuración de \bind (\path{/etc/bind/named.conf}), existe una entrada para zona raíz. Esta entrada especifíca un archivo de sugerencia (hint file) para cada zona raíz, de esta forma el servidor de DNS local conoce la dirección IP de cada servidor raíz.
