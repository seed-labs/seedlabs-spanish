...
\paragraph{Desafíos.} Compared to buffer-overflow attacks on 32-bit 
...
En nuestros ataques de buffer overflow, necesitamos guardar al menos una dirección de memoria en el payload, este será copiado en el stack por medio de la función \texttt{strcpy()}. Sabemos que la función \texttt{strcpy()}  dejará de copiar datos al encontrarse con un valor cero. Por lo tanto, si un cero
aparece en el medio del payload, los datos que siguen después de este no serán copiado. Resolver esta cuestión es uno de los desafíos más difíciles para este ataque.
